        ...
        <div className='lg:pr-20 flex flex-col items-center'>
            <div className='flex flex-row items-center overflow-x-auto'>
                {options.map((value, id) =>
                    <div
                        key={id}
                        className={`cursor-pointer ${value.name.font} ${selected != id ? 'animate-blinking' : ''} hover:animate-none text-2xl sm:text-3xl lg:text-5xl shrink-0 mr-3 lg:mr-10 mb-4 ${value.name.color}`}
